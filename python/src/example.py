...
"""
スクレイピングプログラム：Pythonの組み込み関数をスクレイピングで入手するプログラムです。
"""

__author__ = "Kobayashi Shun"
__version__ = "1.0.0"
__date__ = "2023/09/06 (Created: 2023/09/06)"
...
def main() -> int:
    ...
    table_python3 = table_of_builtin_function_and_hyper_reference("http://docs.python.jp/3.7/library/functions.html")
    ...
    if not os.path.isdir(os.path.join(current_directory, "output")):
        os.mkdir("output")

    a_file = os.path.join(current_directory, "output", "built_in_function.html")
...
def table_of_builtin_function_and_hyper_reference(the_url_string) -> tuple:
    """
    Pythonの組み込み関数のWebページをウェブスクレイピングし、組み込み関数名とハイパーリンクの辞書を作って応答します。
    実際には、Webページを解析して、組み込み関数のtableタグを探し出し、組み込み関数名とハイパーリンクを抽出します。
